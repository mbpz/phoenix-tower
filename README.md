...
- [x] **基础 UI（B-10）**：bevy_egui 积木面板（35 种、点击选中、按本色着色）+ 完成度进度条 + **20 步撤销/重做**（Command 模式）+ CJK 子集字体（16.4MB → 318KB）
...
- [x] **性能验证（B-20）**：实测 M1 集显 50k 积木 ~110 FPS（早前；后续受机器内存压力与 egui 面板影响降至 ~36，真实玩法场景不受影响）；FPS/积木数 HUD + `PHOENIX_STRESS=<n>` 压力模式 + 集合扫描 revision 门控（ADR-007）
...
└── ui/
    ├── hud.rs              # HUD 提示（右下角）
    └── block_panel.rs      # egui 面板：积木/图鉴/成就/存档 + 主题/挑战切换
assets/
├── fonts/                  # CJK 子集字体（B-10）
