...
> 换电脑或新任务接手，请先阅读 [跨电脑接手说明](docs/handoff/README.md)：包含当前进度、可移植白模、环境配置和不随 Git 同步的内容。

基于 **Rust + Bevy 0.19** 的轻量级 3D 积木搭建游戏：以武汉黄鹤楼为主题，玩家通过放置模块化积木**精准复原**、**自由创造**或**挑战重建**这座千年名楼。

> 状态：**可编辑庭院原型，仍在原生试玩与视觉验收中**。自动测试通过不等于成品游戏交付；全流程试玩、完整美术和性能长测仍待完成。
...
```bash
cargo run          # 默认进入可编辑的 Blender 江岸亭庭院
cargo run -- --tower # 原黄鹤楼三步教程
cargo check        # 快速编译检查
...
| `N` | 跳过新手教程（--tower 入口进入三步引导：大台基→高柱→重檐大梁） |
...
- [x] **基础 UI（B-10）**：**Bevy-Lunex 保留模式 UI**（2026-09 重构，替代 bevy_egui）：积木面板（35 种、点击选中、悬停/选中高亮、滚轮滚动）+ 完成度进度条 + 主题/挑战下拉 + 存档 Tab（保存/JSON/分享/导入/路径输入）+ 图鉴/成就 Tab + 知识卡片 toast + 透明度滑杆；**20 步撤销/重做**（Command 模式）+ CJK 子集字体（约 16MB → 433KiB，覆盖代码与积木数据中的中文）
...
![Bevy 原生运行截图：江岸亭](docs/images/courtyard-day.png)

*实际游戏的离屏 3D 截图，不含 UI；不是 Blender 效果图。*

```bash
# 无参数就是庭院，不需要安装 Blender 即可玩
cargo run --locked
# 原塔楼教程入口保留
cargo run --locked -- --tower
```

样板由 **8 个真实积木**组成：石台、四根朱柱、两根梁枋、七格飞檐顶，不是整栋静态模型。
首次进入自由模式并选中亭顶；**Backspace 撤销屋顶 → M 打开蓝图 → 把绿色预览移到亭子上点击重建**。
也可用重做快捷键（各平台修饰键仍需单独验收），左拖环绕、右拖平移、滚轮缩放，T 切换昼夜，F2 截图，F5 保存。
加载自己保存的 `.ptw` 继续编辑时，默认启动后使用原来的存档面板或 F9；初始化不会自动写任何存档。
如果同时指定 `PHOENIX_LOAD` 或 `PHOENIX_STRESS`，导入/压力模式优先，样板不会填充世界。

- 六种 Blender 模型：大台基、高朱柱、梁枋、楼板、斗拱、江亭顶。楼板/斗拱可从原积木面板单独选择。
- 默认庭院启用建筑模型；`--tower`、启动导入与压力模式继续使用程序化网格。启动导入目前不恢复庭院视觉模式，存档数据不受影响。缺少 GLB 文件时仍会警告并回退，不能视为美术验收通过。
- 一个模型只有一个 mesh/primitive：顶点颜色 + 内嵌粗糙度/金属度小图集，区分石、木、红漆、瓦、金饰；无外部贴图或新增依赖。
- 模型尺寸与占格一致，放置/撤销/重做/拆除/存档仍走同一数据路径，存档格式保持 v1。
- 新增短暂装配光环，最多响应同帧 16 个新积木，0.45 秒回收；不改变积木或碰撞体的变换。
- 有边界的水岸、石坪、步道、栏杆、松树与两盏庭院灯不参与占格或物理；环境复用 5 个网格、12 个材质。没有行走、游泳或水面物理。
- 江岸亭蓝图使用 8 个整件预览替代 271 个体素提示，仍按原始占格验证放置。
- 本轮原生验证、实测数据与未通过项见 `docs/refactoring/courtyard-visual-acceptance.md`。
