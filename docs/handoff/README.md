...
- 最新完整 Blender 白模是 **v08：33 个网格，106,008 个三角面**。
  M1 仍为 **revise**，不是竣工实测模型，不是可导出的正式游戏资产；M2–M4 尚未开始。
- v09 在旧机器内存中留下未完成场景，**没有已验证的完整 v09 文件**。
  顶层支承纯几何预检为 16 处可生成、4 个转角净空冲突；修订代码尚未完成 Blender 实建/视觉验收。
- 原先的 MCP 权限审核超时是历史问题。本次 2026-09-08 已成功查询插件并经 MCP 提取、复读 v08，
  证明本次连接可用；没有据此宣称 v09 已修复，也不保证新电脑已安装相同环境。
...
- [evidence](yellow-crane/evidence/)：v08 的拓扑、旧场景保留、变更范围及有限接触采样记录，
  以及 v09 纯几何支承预检。它们是**历史证据**，不是本次重新运行完整建模/渲染得到的结果。
...
2. 阅读支承代码及测试。恢复迭代时重载 `top_support_whitebox` 和 `exterior_whitebox`，
   使用新版本 **10**（如已存在则递增），保留原机器的未完成09。
